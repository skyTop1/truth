...
- `src/api/http.ts` 与 `src/api/modules/*` 当前仅作为预留骨架；没有单独确认前，不要把祭祖、祖先节点、海报记录接到远程请求。
...
- 新增页面或分包页面时，同步更新 `src/pages.json`。
- 修改底部导航时，同步更新 `src/constants/navigation.ts`、`src/types/navigation.ts`、目标页面路径。
- 新增图标时，同步更新 `src/constants/icons.ts`，并优先复用 `src/components/app/AppIcon.vue`。
- 扩展 store 时，同步检查 `src/composables/useTheme.ts` 等依赖 store 的组合式逻辑是否受影响。

## AI 决策规范

1. 先判断改动属于页面、业务组件、composable、store、API、主题还是配置。
2. 再检查仓库内是否已有同类实现可复用，优先复用现有文件和命名。
3. 最后列出必须联动的文件，再开始修改。
4. 如果平台行为可能分歧，先按 `H5` 和 `微信小程序` 评估，再决定是否补条件编译。
...
- 应该做：新增图标时，先去 `icones.js.org` 选语义贴切的 `Lucide` 图标，再补到 `src/constants/icons.ts` 并通过 `AppIcon` 使用。
- 不应该做：直接在组件样式里把 `#355A76` 替换成另一个颜色。
- 不应该做：在页面模板里直接写 `△`、`◎` 或复制粘贴一段零散 SVG。

## 工作流程

- 开工前先找落点文件与联动文件。
- 修改后至少执行一次与改动相关的最小验证，优先 `pnpm run lint` 或 `pnpm run type-check`。
- 交付时说明本次是否涉及 `H5`、`微信小程序`、`App` 差异，并列出未验证路径。
